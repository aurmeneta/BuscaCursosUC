  ...
  return getFetch()
    ...
    .then(html => {
      ...
      filas.forEach(fila => {
        ...
        const vacantesTotales = parseInt($(columnas[13]).text())
        ...
        cursos.push(new Curso(nrc, sigla, permiteRetiro, ingles, seccion, aprobacionEspecial, area, formato,
          categoria, nombre, profesor, campus, creditos, vacantesTotales, vacantesDisponibles, horario))
      })
...
function buscarSigla (periodo, sigla, url = URLS.buscacursos) {
  return obtenerCursos(url + `?cxml_semestre=${periodo}&cxml_sigla=${sigla}`)
}

function buscarProfesor (periodo, profesor, url = URLS.buscacursos) {
  return obtenerCursos(url + `?cxml_semestre=${periodo}&cxml_profesor=${profesor}`)
}

function buscarCurso (periodo, nombre, url = URLS.buscacursos) {
  return obtenerCursos(url + `?cxml_semestre=${periodo}&cxml_nombre=${nombre}`)
}

async function obtenerCurso (periodo, nrc, url = URLS.buscacursos) {
  const resultados = await obtenerCursos(url + `?cxml_semestre=${periodo}&cxml_nrc=${nrc}`)
  return resultados[0]
}
...
async function obtenerPeriodos (url = URLS.buscacursos) {
  ...
  const options = $('select[name="cxml_semestre"]').find('option').toArray()

  return options.map(option =>
    $(option).text().trim().replace(' Primer Semestre', '-1').replace(' Segundo Semestre', '-2').replace(' TAV', '-3')
  )
}
